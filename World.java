...
public class World extends JFrame {
	 ...
	 * Should only called by Turtle class methods 
	 */
	void drawLine(Point2D p1, double newx, double newy, double width, Color color) {
            double oldx = p1.getX();
            double oldy = p1.getY();
            double theta = Math.atan2(newy - oldy, oldx - newx);
            double xOffset = -10.5 * Math.sin(theta);
            double yOffset = -10.5 * Math.cos(theta);
            this.drawLine(new Point2D.Double(oldx + xOffset, oldy + yOffset),  
                    new Point2D.Double(newx + xOffset,newy + yOffset), width, color);
	}
	/**
	 * Should only called by Turtle class methods 
	 */
